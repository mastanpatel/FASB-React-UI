...
const Inventory = (props) => {
  ...
  const generateId = (data) =>
    data.reduce((acc, current) => Math.max(acc, current.FASBInventoryID), 0) +
    1;
  ...
  const removeItem = (data, item) => {
    let index = data.findIndex(
      (p) =>
        p === item ||
        (item.FASBInventoryID && p.FASBInventoryID === item.FASBInventoryID)
    );
    if (index >= 0) {
      data.splice(index, 1);
    }
  ...
  const cancel = (dataItem) => {
    const originalItem = sampleInventory.find(
        ...
        >
          ...
          <Column
            ...
            cell={(props) => {
              ...
              return (
                <td>
                  <button className="k-primary k-button">Edit</button>
                  {/* <input
                                        disabled={false}
                                        type="checkbox"
